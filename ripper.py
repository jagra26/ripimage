...
def join(tiles, width=0, height=0):
    ...
    if width > 0 and height > 0:
        ...
        im = np.zeros([get_combined_size(tiles)[0], get_combined_size(tiles)[1], 3],dtype=np.uint8)
    columns, rows = calc_columns_rows(len(tiles))
    for tile in tiles:
        try:
            im[tile.coords[0]:tile.coords[0]+tile.image.shape[1], tile.coords[1]:tile.coords[1]+tile.image.shape[1]] = tile.image
        except IOError:
...
'''if __name__ == "__main__":
    print("oi")
    tiles = slice("input/nirvana_smile.png", number_tiles=9, save=False)
    save_tiles(tiles, directory="output")
    join(tiles)'''